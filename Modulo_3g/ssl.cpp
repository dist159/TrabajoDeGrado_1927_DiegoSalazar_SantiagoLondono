...
/*******************************************************************************
    ...
    Notas:           Este codigo permite configurar los protocolos para 
                     establecer una comunicacion segura y encriptada (SSL) 
                     (Secure Sockets Layer) entre en modulo UG95 y un servidor.
...
{
	//AT+QSSLOPEN=1,1,4,"goldenchicken-6d6f4.firebaseio.com",443,2
...
bool SSL :: close(unsigned char contexid)
{
	...
	while(flag)
	{
		if(gsm.available())
		{
			...
			{
				String req = gsm.readStringUntil('\n');
				Serial.println("Comunicacion finalizo con exito");
				return(true);
			}
		...
		{
			stopSend();
			Serial.println("close time out: Al finalizar comunicacion");
			previousMillis = currentMillis;
			return(false);
		}			
	}
