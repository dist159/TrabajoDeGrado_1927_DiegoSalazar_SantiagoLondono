...
let AdminPage = class AdminPage {
    constructor(menu, navCtrl, _Servicios, storage, db, modal) {
        this.menu = menu;
        ...
        this._Servicios.userState().subscribe((user) => {
            ...
            const _getUsuario = this._Servicios.getUsuario(user.uid).subscribe(res => {
                ...
                this.db.object("Galpones/" + this.idGalpon + "/fechaUltimaConexion").snapshotChanges().subscribe(action => {
                    let fecha = action.payload.val();
                    //emit value in sequence every 1 second
                    const source = Object(rxjs__WEBPACK_IMPORTED_MODULE_6__["interval"])(10000);
                    //output: 0,1,2,3,4,5....
                    const subscribe = source.subscribe(val => {
                        this.verificarConexion(fecha, action.payload.val());
                    });
                });
                ...
                this.db.object("Galpones/" + this.idGalpon + "/" + 'contador').snapshotChanges().subscribe(action => {
                    ...
                    this.contador = action.payload.val();
